...
final class CLICompatibilityTests: XCTestCase {
    ...
    func testDefaultConfigRoundTripsAndKeepsCleanupOptIn() throws {
        ...
        XCTAssertEqual(config.outputDirectory, "~/meetings")
        XCTAssertTrue(config.disableDiarization)
        XCTAssertEqual(config.diarizationEngine, .sortformer)
    }

    func testDefaultAndLegacyEmptyOutputUseHomeMeetings() throws {
        let expected = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("meetings")
